...
async fn get_scenes(app_handle: tauri::AppHandle) -> Result<Vec<store::GameScene>, String> {
    store::get_scenes(&app_handle).map_err(|e| e.to_string())
}

#[tauri::command]
async fn save_scenes(app_handle: tauri::AppHandle, scenes: Vec<store::GameScene>) -> Result<(), String> {
    store::save_scenes(&app_handle, scenes).map_err(|e| e.to_string())
}

#[tauri::command]
async fn open_config_dir(app_handle: tauri::AppHandle) -> Result<(), String> {
    let dir = store::get_config_dir(&app_handle).map_err(|e| e.to_string())?;
    // 用系统文件管理器打开目录
