...
import { GamingPad, Globe, Translate, Github, AT, Settings02 } from '../icons';
                    ...
                    <button
                        onClick={async () => {
                            try {
                                ...
                                showError('打开配置目录失败: ' + e);
                            }
                        }}
                        className="flex items-center gap-2 px-4 py-2 text-sm text-zinc-600 bg-zinc-100 dark:bg-zinc-800 dark:text-zinc-300 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
                    >
                        <Settings02 className="w-4 h-4 stroke-current" />
                        打开配置文件目录
                    </button>
                    <p className="mt-2 text-xs text-zinc-400">
                        配置文件包含 settings.json（API 密钥、快捷键等）和 scenes.json（游戏场景 Prompt），可直接编辑。
                    </p>
                </div>
            </motion.div>
